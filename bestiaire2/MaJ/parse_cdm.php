...
require_once ("../Libs/functions.php");  // fonction d'affichage du r�sultat du parsing
require_once ("../Libs/inc_affichage.php");  // fonction d'affichage du r�sultat du parsing
...
print("<html><head><title>R�sultat de l'analyse d'une cdm</title></head></body></html>");
...
if(isset($_POST['soumettre'])){
  ...
  while ($lignes[$i]){	
    ...
    if(eregi('[ \t]*Niveau.:.(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur': $pcdm['nivmin']=0;       $pcdm['nivmax']=$mot[2]; break;
	      case 'sup�rieur': $pcdm['nivmin']=$mot[2]; $pcdm['nivmax']=$max_carac; break;
		  case '�gal': $pcdm['nivmin']=$mot[2]; $pcdm['nivmax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*Points.+:.(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur' :  $pcdm['pdvmin']=0;       $pcdm['pdvmax']=$mot[2] ;break;
	      case 'sup�rieur' :  $pcdm['pdvmin']=$mot[2]; $pcdm['pdvmax']=$max_pdv; break;
		  case '�gal' :  $pcdm['pdvmin']=$mot[2]; $pcdm['pdvmax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*D.s.+attaque.+:(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur' :  $pcdm['attmin']=0;       $pcdm['attmax']=$mot[2]; break;
	      case 'sup�rieur' :  $pcdm['attmin']=$mot[2]; $pcdm['attmax']=$max_carac; break;
		  case '�gal' :  $pcdm['attmin']=$mot[2]; $pcdm['attmax']=$mot[2]; break; 
      }
    }
    if(eregi('[ \t]*D.s.+esquive.+:(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur':  $pcdm['esqmin']=0;       $pcdm['esqmax']=$mot[2]; break;
	      case 'sup�rieur':  $pcdm['esqmin']=$mot[2]; $pcdm['esqmax']=$max_carac; break;
		  case '�gal':  $pcdm['esqmin']=$mot[2]; $pcdm['esqmax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*D.s.+d.g.t.+:(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur':  $pcdm['degmin']=0;       $pcdm['degmax']=$mot[2]; break;
	      case 'sup�rieur':  $pcdm['degmin']=$mot[2]; $pcdm['degmax']=$max_carac; break;
		  case '�gal':  $pcdm['degmin']=$mot[2]; $pcdm['degmax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*D.s.+R.g.n.ration.+:(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur':  $pcdm['regmin']=0;       $pcdm['regmax']=$mot[2]; break;
	      case 'sup�rieur':  $pcdm['regmin']=$mot[2]; $pcdm['regmax']=$max_carac; break;
		  case '�gal':  $pcdm['regmin']=$mot[2]; $pcdm['regmax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*Armure :(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch(trim($mot[0])){
	      ...
	      case 'inf�rieur':  $pcdm['armmin']=0;       $pcdm['armmax']=$mot[2]; break;
	      case 'sup�rieur':  $pcdm['armmin']=$mot[2]; $pcdm['armmax']=$max_carac; break;
		  case '�gal':  $pcdm['armmin']=$mot[2]; $pcdm['armmax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*Vue :(.+)\((.+)\)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur': $pcdm['vuemin']=0;       $pcdm['vuemax']=$mot[2]; break;
	      case 'sup�rieur': $pcdm['vuemin']=$mot[2]; $pcdm['vuemax']=$max_carac; break;
		  case '�gal': $pcdm['vuemin']=$mot[2]; $pcdm['vuemax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*Capacit.+:(.+) - Aff.+: (.+)',$lignes[$i],$resultat)){
      ...
      switch (trim($mot[0])){
	      ...
	      case 'inf�rieur': $pcdm['mmmin']=0;       $pcdm['mmmax']=$mot[2]; break;
	      case 'sup�rieur': $pcdm['mmmin']=$mot[2]; $pcdm['mmmax']=$max_mag; break;
		  case '�gal': $pcdm['mmmin']=$mot[2]; $pcdm['mmmax']=$mot[2]; break;
      }
    }
    if(eregi('[ \t]*R.sistance.+:(.+)\((.+)\)',$lignes[$i],$resultat)){
    	$pcdm['rmcom'] = trim($resultat[1]);
			...
			switch (trim($mot[0])){
				...
				case 'inf�rieur': $pcdm['rmmin']=0;       $pcdm['rmmax']=$mot[2]; break;
				case 'sup�rieur': $pcdm['rmmin']=$mot[2]; $pcdm['rmmax']=$max_mag; break;
				case '�gal': $pcdm['rmmin']=$mot[2]; $pcdm['rmmax']=$mot[2]; break;
			}
		}
		...
		if(eregi('[ \t]*Dur.e DLA :(.+)\((.+)\)',$lignes[$i],$resultat)){
		  $pcdm['dlacom'] = trim($resultat[1]);
			...
			switch (trim($mot[0])){
				...
				case 'inf�rieur': $pcdm['dlamin']=0;       $pcdm['dlamax']=$mot[2]; break;
				case 'sup�rieur': $pcdm['dlamin']=$mot[2]; $pcdm['dlamax']=$max_carac; break;
				case '�gal': $pcdm['dlamin']=$mot[2]; $pcdm['dlamax']=$mot[2]; break;
			}
		}
	...
	/* Si la cdm est directement envoy�e par Firemago
	   on prend le troll connect� */
	if ($pcdm['troll_id'] == "") {
  ...
  // on applique chaque template au nom du monste trouv� et on v�rifie si une
  // race correspond.
  $trouve=false;
  ...
  // r�cup�ration du genre de la race pour calculer l'�ge
  $pcdm['genre_race']=$best_races[$pcdm['race']]['genre_race'];
  // v�rification du nom de la famille
  $trouve=$trouve&&array_key_exists($pcdm['famille'],$best_familles);
  if(!$trouve) die("Famille inconnue");
  
  // v�rification de la correspondance famille/race
  $id_famille=$best_familles[$pcdm['famille']]['id_famille'];
  $trouve=$trouve&&($id_famille==$best_races[$pcdm['race']]['id_famille_race']);
  if(!$trouve) die("la famille ne correspond pas � la race");

  // v�rification du nom de l'�ge
  // on cherche si l'�ge indiqu� correspond
  $trouve=$trouve&&in_array($pcdm['age'],$best_ages_nom[$id_famille][$pcdm['genre_race']]);
  if(!$trouve) die("l'�ge ne correspond pas � la famille : ".$pcdm['age']."/".$pcdm['famille']." (".$id_famille.")");

  // Tout est ok, on r�cup�re les clefs
