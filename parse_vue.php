	                    ...
	                    case 70: # Position
	                        ...
	                        if (preg_match("/affichage est limité à (.+) cases horizontalement et (.+) verticalement/", $line, $parts)) { # Vue
	                            if (($parts[1] != $parts[2] * 2) && ($parts[1] + 1 != $parts[2] * 2)) {
	                                die(" Le nombre de cases vues horizontalement doit être le double du nombre de cases vues verticalement (-1 si n&eacute;cessaire)."."<br /> Ajustez vos limites et recommencez.");
	                            }
	                            $nCasesVue = $parts[1];
